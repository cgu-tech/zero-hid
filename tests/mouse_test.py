...
from zero_hid.hid.mouse import raise_mouse_event, MOUSE_BUTTON_LEFT, MOUSE_BUTTON_RIGHT, MOUSE_BUTTON_MIDDLE
from common import read_bytes, temp_path

def raise_mouse_event_data(buttons, x, y, vertical_wheel_delta, horizontal_wheel_delta):
    with temp_path() as dev_path:
        with open(dev_path, "w+b") as dev:
            raise_mouse_event(dev, buttons, x, y, vertical_wheel_delta, horizontal_wheel_delta)
            dev.seek(0)
            data = dev.read()
    return data
...
def test_horizontal_scroll_4():
    data = raise_mouse_event_data([], 0, 0, 0, 4)
