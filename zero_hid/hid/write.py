...
def write_to_hid_interface(hid_dev, buffer):
    ...
    future = _thread_pool.submit(_write_to_hid_interface_immediately, hid_dev, buffer)
    try:
        ...
        future.cancel()
        raise WriteError(
            f"Timed out writing to HID interface: {hid_dev}. Is USB cable connected and Gadget module installed? check https://git.io/J1T7Q"
        )
    except Exception as e:
        raise WriteError(
            f"Failed to write to HID interface: {hid_dev}. Reason: {e}"
        ) from e
